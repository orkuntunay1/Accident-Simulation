...
# -*- coding: utf-8 -*-
"""
Created on Mon May  2 08:54:18 2022
...
def move_to_city(towing_v, towing_v_loc, time_min, avg_speed,city_center_loc):   
    ...
    for i in range(int(len(towing_v)/2),len(towing_v)):
        #Move the vehicle if the vehicle is available within the period
        if towing_v[i] == 1:
...
#Coordinates of the Two Hubs       

loc = data_n[(data_n["Within Citi Center (20KM)"]== True)][[
    "Pick-up Longitude","Pick-up Latitude"]].values           
clf = KMeansConstrained(
     n_clusters=2,
     size_min= int(len(loc)/2)-1,
     size_max = int(len(loc)/2)+1,
     random_state=0
)
clf.fit_predict(loc)   

#Set the hybrid queuing parameter
q_param = 10
        ...
        while np.sum(event["Status"])>0:
            ...
            if event.at[i,"Type"] == "Vehicle Available":
                ...
                if len(queue) > 0:
                    ...
                    if len(queue) < q_param:
                        acc_closest_i = closest_acc(tow_i,towing_v_loc,queue,event)
                        #Process the closest accident to the available vehicle
                        acc = queue.pop(acc_closest_i)
                    else:
                        #FIFO, if queue length is taller than the threshold
                        acc = queue.pop(0)
...
result.to_csv("Result_Hybrid_2Hubs_q10_1000sim_Tow_3to7.csv")
